...
- **CI runs the product on every push & PR** (@.github/workflows/ci.yml): backend ruff + pytest + a real-Postgres integration job, and frontend build + lint + vitest; the backend installs from the pinned lock and a `lockfile-drift` job guards it. Still run `/verify` locally before committing.
...
- **Backend lockfile:** `supply-os-v1/requirements.txt` (runtime, used by Railpack) and `requirements-dev.txt` (runtime + dev, used by CI) are **fully pinned**, generated by `pip-compile` from `pyproject.toml` — which stays the loose source of truth. Don't hand-edit the generated `.txt` files. **Regenerate on Linux x86_64 + Python 3.12** after any dependency edit — that is the deploy target (Railway + CI are both Linux x86_64). Do NOT generate on macOS: `pip-compile` resolves `platform_machine` markers for the host, and macOS arm64 silently drops Linux-only deps (e.g. SQLAlchemy's `greenlet`), which the `lockfile-drift` CI job then flags. Portable command (no local Linux box needed; pinned pip-tools keeps the header byte-stable):
  ```bash
  docker run --rm --platform linux/amd64 -v "$PWD/supply-os-v1":/w -w /w python:3.12 bash -c "\
  ...
  ```
  (Or run the two `pip-compile --strip-extras …` commands on any Linux x86_64 host / the CI runner. The `lockfile-drift` job recompiles on Linux and `git diff --exit-code`s — it is the source of truth.)
