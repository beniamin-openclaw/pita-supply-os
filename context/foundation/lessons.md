...
## pip-compile locks are platform-specific — generate on the deploy target's platform

- **Context**: Generating or refreshing a pip-tools lock (`supply-os-v1/requirements.txt` + `requirements-dev.txt`) for a Linux deploy (Railway + CI are both Linux x86_64) from a macOS (Apple Silicon / arm64) dev machine.
- **Problem**: `pip-compile` resolves environment markers (`platform_machine`, `python_version`) for the HOST it runs on. macOS arm64 reports `platform_machine == "arm64"`, which is NOT in SQLAlchemy's `greenlet` marker (`x86_64 / aarch64 / amd64 / win32`), so the macOS-generated lock silently OMITTED `greenlet` — a package the Linux runtime resolves. Local recompiles agreed with each other (same host) so local proof was green; the real-CI `lockfile-drift` job (Linux) was the only thing that caught it (H-01).
- **Rule**: Generate/refresh pip-tools locks on the deploy target's platform — Linux x86_64 via `docker run --platform linux/amd64 python:3.12 …`, a Linux host, or the CI runner — never macOS. Treat a target-platform `lockfile-drift` CI job as the source of truth; a local cross-platform recompile cannot validate it. (pip-tools has no universal/cross-platform resolution; uv does, if a single universal lock is ever wanted.)
- **Applies to**: implement, impl-review
