...
| H-01 | quality-hardening           | Backend lockfile + TS strict + mypy/pyright + thicker ruff — reproducible, type-safe  | —                | `health-check.md` #3/#4/#5/#6   | implemented on PR #27 (unmerged) |
...
## Horizon 3 — Rollout & operations (Jul–Sep 2026)

Horizon 2 shipped the infrastructure (Supabase, multi-supplier data, product CI); Horizon 3 is what actually happened next as the pilot scaled into daily operations. Four more locations went live (Bracka, Norblin, KEN, Browary); the Manager gained a **Transport** workflow to combine several locations' orders into one supplier pickup, replacing the legacy "Ordering PB v5" spreadsheet; five-plus rounds of operator/captain feedback corrected master data and UX; and a first **dynamic (usage-based) target** replaced a static number for Wola. None of this was planned as roadmap slices — each landed as its own change-folder lane, several outside the F-0x/S-0x numbering, so this section reconciles that work into the roadmap after the fact rather than re-litigating it as new planning.

### At a glance — Horizon 3

| ID   | Change ID                          | Outcome (user/owner can …)                                                                                                                                                                    | Refs                                                           | Status                                          | Where                                                              |
| ---- | ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------------ | ------------------------------------------------------------------- |
| R-01 | feedback-r4-suppliers-master-data   | Owner gets supplier/master-data corrections from Intermlecz / Blue Service / Coca-Cola / Bukat order feedback, so suggestions hold again                                                        | FR-012                                                          | done                                              | `context/archive/2026-07-15-feedback-r4-suppliers-master-data/`     |
| R-02 | feedback-r5-drafts-statuses         | Captain keeps an order draft across sessions; order shows post-receipt statuses; supplier emails carry an NIP footer                                                                             | FR-006, FR-010, FR-011                                          | done                                              | `context/archive/2026-07-16-feedback-r5-drafts-statuses/`           |
| R-03 | feedback-r6-beverage-packs-edit-view| Captain edits an order the same way they create one (full prefilled product list); beverages order in packs (zgrzewki)                                                                          | FR-012 (units)                                                  | done                                              | `context/archive/2026-07-16-feedback-r6-beverage-packs-edit-view/`  |
| R-04 | feedback-r7-tushar                  | Supplier dispatch email adds `biuro@` as DW (cc) and a blank delivery-date line for the supplier to fill in                                                                                       | FR-009, FR-011                                                  | done                                              | `context/archive/2026-07-25-feedback-r7-tushar/`                    |
| R-05 | bracka-rollout                      | Bracka location goes live end-to-end (min/max from CSV, Captain token, Manager queue no longer hardcoded to Wola)                                                                                | PRD "+2 locations" gated rollout                                | done                                              | `context/archive/2026-08-18-bracka-rollout/`                        |
| R-06 | norblin-rollout                     | Norblin location goes live end-to-end (new location, min/max from CSV, Captain token)                                                                                                            | PRD "+2 locations" gated rollout                                | done                                              | `context/archive/2026-08-18-norblin-rollout/`                       |
| R-07 | three-missing-products              | Three missing products (paper trays/cups, burger patties) added to the catalog                                                                                                                    | FR-012                                                          | done                                              | commit `23dbb78` (no change folder)                                  |
| R-08 | wolska-blueservice-master-data      | 9 new Blue Service products + Wola thresholds for paper trays go live (Track A of the supplier-per-location split)                                                                                | FR-012                                                          | done                                              | `context/archive/2026-08-20-wolska-blueservice-master-data/`        |
| R-09 | ken-browary-master-data             | KEN and Browary thresholds corrected from real inventory CSVs (KEN de-duplicated from a Wola copy; Browary activated for ordering)                                                               | FR-012                                                          | done                                              | `context/archive/2026-08-31-ken-browary-master-data/`               |
| R-10 | to-ordering-pago                    | Manager combines several locations' orders into one Pago transport order (Transport screen), with driver/vehicle logistics, print/PDF driver docs, and per-user Gmail drafts (v1–v5.6)           | FR-013 (channel-aware dispatch); "Pago internal pipeline" non-goal, now partially addressed | done                                              | `context/archive/2026-08-21-to-ordering-pago/`                      |
| R-11 | training-feedback-0901              | Post-training fixes across ordering UX, audited inventory edits, and Pago pickup scoping for the 4 newly onboarded locations                                                                     | operator feedback, 2026-09-01 training                          | done                                              | `context/archive/2026-09-01-training-feedback-0901/`                |
| R-12 | rolki-minima-master-data            | Per-location paper-roll (rolki) quantities and supplier minimums corrected in prod master data                                                                                                    | FR-012                                                          | done                                              | `context/archive/2026-09-05-rolki-minima-master-data/`              |
| R-13 | pack-units-display-mobile-wrap      | Captain/Manager order and inventory screens show pack-unit equivalents (N szt = M zgrzewek) and wrap instead of clipping on phones                                                                | FR-002/FR-003 (units); mobile UX                                | done                                              | `context/archive/2026-09-06-pack-units-display-mobile-wrap/`        |
| R-14 | inventory-confirm-and-history       | Captain sees a confirmation card after submitting an inventory count and a combined Orders/Inventories history view; gyros split and Corfu Pilsner corrected in master data                      | FR-015/FR-016/FR-018/FR-019                                     | done                                              | `context/archive/2026-09-06-inventory-confirm-and-history/`         |
| R-15 | week1-feedback-targets              | New "stock until next delivery" reason code, KEN/Wola target corrections, bifteki fix, and communication tweaks from week-1 captain feedback                                                      | FR-004 (deviation reasons), FR-012                              | done                                              | `context/archive/2026-09-06-week1-feedback-targets/`                |
| R-16 | finance-invoice-reconciliation      | Manager reconciles goods receipts against Symfonia eBiuro invoices on a KEN pilot ("Faktury vs dostawy")                                                                                          | "finance/KSeF" non-goal, now partially addressed                | done                                              | `context/archive/2026-09-06-finance-invoice-reconciliation/`        |
| R-17 | dynamic-target-wola                 | Wola's Pago/Coca-Cola targets are computed from real usage × days-to-delivery instead of a static number, with a kill switch                                                                     | "predictive AI"/"GoStock" non-goals, now partially addressed    | implemented — PR #30 open, prod SQL pending operator approval | PR #30                                                              |
| R-18 | supplier-per-location               | (parked) Per-location supplier choice was implemented and reviewed, then the office-supply pin mechanism was dropped 2026-08-21 when it didn't match real data; `supplier_products.active` still isn't enforced | Data section ("no schema change in the baseline pilot")        | in-flight — pin mechanism dropped 2026-08-21, PR #26 parked | `context/changes/supplier-per-location/`                            |
| R-19 | multi-location-master-data + H-01   | Per-location master data reconciled from 12 inventory sheets across 8 locations (reconciliation engine + onboarding batches, additive-inactive); bundles H-01 hardening (TS strict, backend lockfile, wider ruff, advisory pyright) | H-01 (`health-check.md` #3/#4/#5/#6)                            | implemented on PR #27, unmerged                  | PR #27                                                              |
...
- ~~**API error-message localization (PL)**~~ — **Done (shipped + archived 2026-06-22)** → `context/archive/2026-06-17-api-error-localization-pl/`. Tier 1 delivered: a frontend localizer (`src/i18n/apiErrors.ts`) maps the Pydantic 422 `type` (+ ctx) to PL/EN copy at the point `ApiError.detail` is built, so all ~15 display sites get it for free; business-rule 400 string details keep the English fallback (Tier 2 — still needs backend error *codes*). Also added a UI guard blocking empty-order submit before the network. Source: owner smoke-check 2026-06-16.
...
- **GoStock integration, receiving/WZ, finance/KSeF, predictive AI** — Why parked: PRD Non-Goals (roadmap postponements). Horizon 1 non-goal, partially taken up in Horizon 3: the finance mirror shipped 2026-09-07 (`finance-invoice-reconciliation`) and GoStock-seeded usage feeds the dynamic target (`dynamic-target-wola`, PR #30).
...
- **order-cancel-with-trace: Manager soft-cancels a pre-dispatch order with a who/when/why trace.** — Archived 2026-06-22 → `context/archive/2026-06-17-order-cancel-with-trace/`. Not a formal slice (from the `manager-ux-feedback-backlog` memory). `captain_submitted|manager_claimed → cancelled` (never hard-deleted), trace in 3 new `orders` columns (migration 0004, applied to prod Supabase before the code deploy); Manager-only button mirroring release. Lesson: a new column referenced by `_ORDER_COLUMNS` breaks the WHOLE Supabase integration suite if the test `_schema` fixture doesn't apply the migration — impl-review caught it (locally hidden because integration tests are deselected without a live DB); always wire a new migration into the fixture + add `ADD COLUMN IF NOT EXISTS`.
